...
(claude-code-ide-make-tool
    ...
    :name "ast-rewrite"
    :description "Structural find-and-replace over the syntax tree via ast-grep CLI, not over buffer text. Prefer over Edit whenever same shape occurs more than once, indentation uncertain, or lookalike text sits nearby -- the three ways a textual edit lands in the wrong place. Pattern and rewrite are code fragments: $NAME binds one node and expands again in rewrite (foo($A, $B) -> bar($B, $A)), $$$NAME binds a node list. Whitespace and line breaks ignored, node kinds must agree, text inside comments or strings never matches. Rewrites only -- search with grep-block. Args: pattern, rewrite, path (file or dir), lang (language name; needed to explain a failing pattern), globs (gitignore-style filter, ! to exclude), strictness (cst | smart | ast | relaxed | signature | template), dry_run (diff without writing). Dry pass always runs first: a pattern matching nothing returns its own parse tree instead of silence, and nothing is written while a target file has unsaved changes in Emacs. Rewritten files revert in Emacs automatically."
    :args '((:name "pattern"
             :type string
