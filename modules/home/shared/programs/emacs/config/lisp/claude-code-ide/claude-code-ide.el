...
(defcustom claude-code-ide-buffer-name-function #'claude-code-ide--default-buffer-name
  "Function to generate buffer names for the AI session.
...
(defcustom claude-code-ide-open-code-model nil
  "Model to use with opencode (--model flag).
If nil, uses opencode's default model."
  :type '(choice (const :tag "Default" nil)
                 (string :tag "Model name (e.g. provider/model)"))
  :group 'claude-code-ide)

(defcustom claude-code-ide-open-code-agent nil
  "Agent to use with opencode (--agent flag).
If nil, uses opencode's default agent."
  :type '(choice (const :tag "Default" nil)
...
(defun claude-code-ide--opencode-p ()
...
(defun claude-code-ide--default-buffer-name (directory)
  ...
  (format "%s%s]*"
          (claude-code-ide--backend-buffer-prefix)
...
(defun claude-code-ide--set-process (process &optional directory)
  ...
  (when (= (hash-table-count claude-code-ide--processes) 0)
    ...
    ;; Backends without a resize handler (ghostel) return nil.
...
(defun claude-code-ide--cleanup-on-exit (directory)
  "Clean up MCP server and process tracking when session exits for DIRECTORY."
  (unless claude-code-ide--cleanup-in-progress
    ...
    (unwind-protect
        (progn
          ...
          (unless (claude-code-ide--opencode-p)
            ...
            (when-let ((buffer (get-buffer buffer-name)))
              (when (buffer-live-p buffer)
                (let ((kill-buffer-hook nil) ; Disable hooks to prevent recursion
                  ...
                  (kill-buffer buffer)))))
          (claude-code-ide-debug "Cleaned up %s session for %s"
                                 (claude-code-ide--backend-name)
                                 (file-name-nondirectory (directory-file-name directory))))
...
(defun claude-code-ide--detect-cli ()
  "Detect if the selected backend CLI is available."
  (let* ((cli-path (claude-code-ide--backend-cli-path))
         (available (condition-case nil
                        (eq (call-process cli-path nil nil nil "--version") 0)
                      (error nil))))
    (setq claude-code-ide--cli-available available)))

(defun claude-code-ide--ensure-cli ()
  "Ensure the selected backend CLI is available, detect if needed."
  ...
  (let ((window (get-buffer-window existing-buffer)))
    (if window
        ...
        (progn
          ...
          (claude-code-ide-debug "%s window hidden" (claude-code-ide--backend-name)))
      ...
      (progn
        ...
        (claude-code-ide-debug "%s window shown" (claude-code-ide--backend-name))))))
...
Additional flags from `claude-code-ide-cli-extra-flags' are also included."
  (let ((cmd "opencode"))
    ...
    (when continue
      (setq cmd (concat cmd " -c")))
    ;; Add model if configured
    (when claude-code-ide-open-code-model
      (setq cmd (concat cmd " -m " (shell-quote-argument claude-code-ide-open-code-model))))
    ;; Add agent if configured
    (when claude-code-ide-open-code-agent
      (setq cmd (concat cmd " --agent " (shell-quote-argument claude-code-ide-open-code-agent))))
    ;; Add any extra flags
    (when (and claude-code-ide-cli-extra-flags
               (not (string-empty-p claude-code-ide-cli-extra-flags)))
      (setq cmd (concat cmd " " claude-code-ide-cli-extra-flags)))
    ;; Register MCP tools in opencode.jsonc
    (when (claude-code-ide-mcp-server-ensure-server)
...
(defun claude-code-ide--create-terminal-session (buffer-name working-dir port continue resume session-id)
  "Create a new terminal session for the selected AI CLI.
    ...
    (cond
     ...
     ((eq claude-code-ide-terminal-backend 'vterm)
      (let* ((vterm-buffer-name buffer-name)
              ;; Set vterm-shell to run the AI CLI directly
              (vterm-shell ai-cmd)
     ...
     ((eq claude-code-ide-terminal-backend 'eat)
      (let* ((buffer (get-buffer-create buffer-name))
             ...
             (cmd-parts (claude-code-ide--parse-command-string ai-cmd))
     ...
     ;; ghostel backend
     ((eq claude-code-ide-terminal-backend 'ghostel)
      (let* ((cmd-parts (claude-code-ide--parse-command-string ai-cmd))
...
(defun claude-code-ide--start-session (&optional continue resume)
  "Start a session for the current AI backend.
If CONTINUE is non-nil, pass the continue flag.
If RESUME is non-nil, pass the resume flag.
...
- Process and buffer lifecycle management"
  (unless (claude-code-ide--ensure-cli)
    (user-error "%s CLI not available. Please install it and ensure it's in PATH"
                (claude-code-ide--backend-name)))
      ...
      (let ((port nil)
        ...
        (condition-case err
            (progn
              ...
              (setq port (if (claude-code-ide--opencode-p)
                             0
                           (claude-code-ide-mcp-start working-dir)))
              ...
              (let* ((buffer-and-process (claude-code-ide--create-terminal-session
                                        ...
                                        (lambda (proc event)
                                          ...
                                          (when (string-match "exited abnormally with code \\([0-9]+\\)" event)
                                            (let ((exit-code (match-string 1 event)))
                                              (claude-code-ide-debug "%s process exited with code %s, event: %s"
                                                                     (claude-code-ide--backend-name) exit-code event)
                                              (message "%s exited with error code %s"
                                                       (claude-code-ide--backend-name) exit-code)))
                ...
                (claude-code-ide-log "%s %sstarted in %s%s"
                                     (claude-code-ide--backend-name)
          ...
          (error
           ...
           (when (and port (not (claude-code-ide--opencode-p)))
             (claude-code-ide-mcp-stop-session working-dir))
           ;; Restore opencode.jsonc if we modified it
           (when (claude-code-ide--opencode-p)
             (claude-code-ide--opencode-remove-mcp))
...
(defun claude-code-ide ()
  "Run the selected AI CLI in a terminal for the current project or directory."
  (interactive)
...
(defun claude-code-ide-check-status ()
  "Check the selected AI CLI status."
  (interactive)
    ...
    (if claude-code-ide--cli-available
        (let ((version-output
               (with-temp-buffer
                 ...
                 (buffer-string))))
          (claude-code-ide-log "%s CLI version: %s" backend-name (string-trim version-output)))
      (claude-code-ide-log "%s is not installed." backend-name))))

;;;###autoload
(defun claude-code-ide-stop ()
  "Stop the AI CLI session for the current project or directory."
  (interactive)
  (let* ((working-dir (claude-code-ide--get-working-directory))
         (buffer-name (claude-code-ide--get-buffer-name)))
    (if-let ((buffer (get-buffer buffer-name)))
        (progn
          ...
          (kill-buffer buffer)
          (claude-code-ide-log "Stopping %s in %s..."
                               (claude-code-ide--backend-name)
                               (file-name-nondirectory (directory-file-name working-dir))))
      (claude-code-ide-log "No %s session is running in this directory"
                           (claude-code-ide--backend-name)))))


;;;###autoload
(defun claude-code-ide-switch-to-buffer ()
  "Switch to the AI CLI buffer for the current project.
  ...
  (let ((buffer-name (claude-code-ide--get-buffer-name)))
    (if-let ((buffer (get-buffer buffer-name)))
        (if-let ((window (get-buffer-window buffer)))
          ...
          (claude-code-ide--display-buffer-in-side-window buffer))
      (user-error "No %s session for this project.  Use M-x claude-code-ide to start one"
                  (claude-code-ide--backend-name)))))
...
(defun claude-code-ide-list-sessions ()
  ...
  (let ((sessions '()))
    (maphash (lambda (directory _)
             ...
             claude-code-ide--processes)
    (if sessions
        (let ((choice (completing-read (format "Switch to %s session: " (claude-code-ide--backend-name))
                                       sessions nil t)))
          (when choice
      ...
      (claude-code-ide-log "No active %s sessions" (claude-code-ide--backend-name)))))

;;;###autoload
(defun claude-code-ide-insert-at-mentioned ()
  "Insert selected text into the AI prompt."
  (interactive)
  (if-let* ((project-dir (claude-code-ide-mcp--get-buffer-project))
      ...
      (progn
        ...
        (claude-code-ide-debug "Sent selection to %s" (claude-code-ide--backend-name)))
    (user-error "%s is not connected.  Please start it first" (claude-code-ide--backend-name))))

;;;###autoload
(defun claude-code-ide-send-escape ()
  "Send escape key to the AI CLI terminal buffer for the current project."
  (interactive)
  (let ((buffer-name (claude-code-ide--get-buffer-name)))
    (if-let ((buffer (get-buffer buffer-name)))
        (with-current-buffer buffer
...
(defun claude-code-ide-insert-newline ()
  "Send newline (backslash + return) to the AI CLI terminal buffer for the current project."
  (interactive)
  (let ((buffer-name (claude-code-ide--get-buffer-name)))
    (if-let ((buffer (get-buffer buffer-name)))
        (with-current-buffer buffer
...
(defun claude-code-ide-send-prompt (&optional prompt)
  "Send a prompt to the AI CLI terminal.
...
When called programmatically, sends the given PROMPT string."
  ...
  (let ((backend-name (claude-code-ide--backend-name))
        (buffer-name (claude-code-ide--get-buffer-name)))
    (if-let ((buffer (get-buffer buffer-name)))
        (let ((prompt-to-send (or prompt (read-string (format "%s prompt: " backend-name)))))
          (when (not (string-empty-p prompt-to-send))
            (with-current-buffer buffer
              ...
              (sit-for 0.1)
              (claude-code-ide--terminal-send-return))
            (claude-code-ide-debug "Sent prompt to %s: %s" backend-name prompt-to-send)))
      (user-error "No %s session for this project" backend-name))))

;;;###autoload
(defun claude-code-ide-toggle ()
  "Toggle visibility of the AI CLI window for the current project."
  (interactive)
  (let* ((working-dir (claude-code-ide--get-working-directory))
         (buffer-name (claude-code-ide--get-buffer-name))
...
(defun claude-code-ide-toggle-recent ()
  "Toggle visibility of the most recent AI CLI window.
If any such window is visible, hide all of them.
If none are visible, show the most recently accessed one."
  (interactive)
  (let ((found-visible nil))
    ...
    (cond
     ...
     (found-visible
      (message "Closed all %s windows" (claude-code-ide--backend-name)))
     ...
     ((and claude-code-ide--last-accessed-buffer
      ...
      (message "Opened most recent %s session" (claude-code-ide--backend-name)))
