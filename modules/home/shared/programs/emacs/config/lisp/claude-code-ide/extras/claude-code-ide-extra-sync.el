...
(claude-code-ide-make-tool
    ...
    :name "file-changed"
    :description "Tell Emacs and its language servers a file changed on disk, so a later lsp-def/lsp-refs/getDiagnostics call sees the edit, not stale text. A buffer visiting the file is reverted (eglot then sends textDocument/didChange); a file no buffer visits is announced to every language server owning it via workspace/didChangeWatchedFiles. A buffer with unsaved changes is left alone and reported. Call after editing a file through anything other than Emacs -- eglot suppresses its own watch notification while a buffer visits the file, and drops watches entirely once eglot-max-file-watches is reached, so neither route is guaranteed. Args: file (absolute path), change (created | changed (default) | deleted)."
    :args '((:name "file"
             :type string
