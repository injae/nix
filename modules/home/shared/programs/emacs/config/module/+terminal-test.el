...
(declare-function +vterm--hangul-erase-preedit "+terminal")
(declare-function +vterm--hangul-compose-key "+terminal")
(declare-function +vterm--hangul-decompose-backspace "+terminal")
(declare-function +vterm--self-insert-with-im "+terminal")
(declare-function +vterm-toggle-korean "+terminal")
(defvar +vterm--hangul-preedit-sent)
(defvar +vterm--hangul-captured)
(defvar +vterm--hangul-active)
(declare-function +vterm--after-evil-insert-clear-imf "+terminal")
...
(ert-deftest +vterm-space/with-preedit-flushes-then-sends-space ()
  "Space with preedit: clears state then sends space via orig-fn."
  (+vterm-test--env
    (setq +vterm--hangul-preedit-sent "가")
    (aset hangul-queue 0 1)
    (let (sent orig-called)
...
;;; Non-hangul key flush (punctuation, numbers — the (t ...) cond branch)
...
(ert-deftest +vterm-flush-key/number-with-preedit ()
  "Number key with active preedit: flush state, call orig-fn."
  (+vterm-test--with-ops '() '()
    (setq +vterm--hangul-preedit-sent "ㄱ")
    (let (orig-called)
      (+vterm-test--press ?1 (lambda () (setq orig-called t)))
      (should orig-called)
      (should (equal +vterm--hangul-preedit-sent ""))
      (should-not sent))))
...
(declare-function +vterm--send-backspace-with-im "+terminal")
(declare-function +vterm--send-return-flush-im "+terminal")
