...
## Step 4 — Clipboard + commit handoff

If Emacs MCP tools are available, run in parallel:
- `executeCode` — `(kill-new "<the commit message>")`
- `git-prepare-commit` — with the message pre-filled

If those tools are not available in Pi, print the commit message and ask the user whether to copy/use it manually.

Tell user: press **C-c C-c** to commit when Magit is open.
