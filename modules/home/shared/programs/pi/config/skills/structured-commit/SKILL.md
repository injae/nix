...
2. **MANDATORY: invoke `/skill:commit` now** — do NOT write the message manually. The commit skill reads the staged diff and generates the message. Run its Steps 3–4. If Emacs MCP tools are unavailable in Pi, print the generated message for manual use.
...
- If Emacs MCP tools are available, open Magit status: `executeCode` — `(magit-status)`. Otherwise print `git status --short`.
