;;; +evil.el --- Summery -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:
...
(use-package general
    :custom (general-override-states
                '(insert emacs hybrid normal visual motion override operator replace))
    :config
    (general-evil-setup :with-shortname-maps)
    (general-create-definer leader
        :keymaps '(global override) :states '(n v) :prefix "SPC")
    (leader "<SPC>" 'execute-extended-command
            ...
            "en"    '((lambda ()(interactive) (find-file (f-join user-nix-directory "flake.nix"))) :wk "nix flake")
            "eh"    '((lambda ()(interactive) (find-file (f-join user-nix-directory "home/default.nix"))) :wk "nix home")
            "ef"    '((lambda ()(interactive) (find-file (f-join user-mutable-emacs-directory "config/init.el"))) :wk "configure file")
            "wf"    '(toggle-frame-maximized :wk "Full Screen")
...
(use-package evil-extra-operator :after (evil fold-this)
